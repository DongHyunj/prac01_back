...
*       가져온 데이터가 관계를 맺은 데이터를 추가로 가져오기 위해서 N번의 쿼리를 더 실행하는 문제
*
*       ex)                                                 결과)
...
*           SELECT * FROM reply_likes WHERE reply_idx = 1;
*
*   원인
*       board 객체를 가져왔을 때 JPA에서는 관계를 맺은 객체(reply)는 실제 reply 객체가 아니라
...
*   JPA 서버 개발하는 순서
    ...
    2. ~~~ToOne
        Fetch Join으로 최적화
    3. ~~~ToMany
        1. 페이징 처리가 필요 없고 관계가 1개만 맺었을 때
            Fetch Join으로 최적화
        2. 페이징 처리가 필요, 페이지 번호 필요
            @BatchSize로 최적화, Repository에서 반환 Page
        3. 페이징 처리가 필요, 페이지 번호 필요 없음
            @BatchSize로 최적화, Repository에서 반환 Slice
        4. 읽기 성능이 중요한 경우
            Dto로 처리 방식으로 최적화
* */

public interface BoardRepository extends JpaRepository<Board, Long> {
    // Repository 통해서 Dto에 바로 데이터 담기
    // 성능이 제일 좋음
    @Query("""
    ...
    // 페이징 처리
    @Query("""
            SELECT new com.example.demo.board.model.BoardDto$ListRes(b.idx, b.title, b.user.name, (SELECT COUNT(r) FROM Reply r WHERE r.board.idx = b.idx), b.likesCount) 
            FROM Board b
            """)
    List<BoardDto.ListRes> findAllDtoWithPage(Pageable pageable);
...
//    List<Board> findAll();
    ...
    // UPDATE문을 실행하기 위해서 SELECT로 조회하는거니까 내가 UPDATE할 때까지 다른 애들이 조회 못하게 잠금
